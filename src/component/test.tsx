...
export function Home() {
  ...
  useEffect(() => {
    ...
    const continents = [
      // Gauche (Vert)
      { name: "", x: 0.15, y: 0.4, color: "rgba(34, 197, 94, 0.8)" },
      // Haut (Jaune)
      { name: "", x: 0.5, y: 0.25, color: "rgba(234, 179, 8, 0.8)" },
      // Droite (Rouge)
      { name: "", x: 0.85, y: 0.4, color: " rgba(239, 68, 68, 0.8)" },
      // Centre (Logo) - Monté à y: 0.45 pour être sous le texte
      { name: "", x: 0.5, y: 0.45, color: "white", center: true },
    ];
    ...
    const animate = () => {
      ...
      continents.forEach((continent) => {
        ...
        if (continent.center) {
          const size = 100; // TAILLE AUGMENTÉE
